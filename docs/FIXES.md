...
- [x] `toString()` ignores `localtime_s` / `localtime_r` failure before formatting the result. (`src/domain/scenario_perf.h:82`, `ScenarioRunId::toString`)
...
- [x] A scenario display name is fixed by its first insertion for a given hash. Confirmed intended (not a bug): the hash is the scenario's identity and the name must not drift; covered by `ScenarioListRetainsTheFirstObservedDisplayNameForASharedHash`. (`src/domain/user_profile.cpp:27`, `UserProfile::addScenarioPerf`; `src/domain/scenario_perf.h:53`, `ScenarioId::operator==`)
...
- [x] `getAverageScore()` uses a float accumulator with avoidable precision drift. Accumulator is now `double`, cast to `float` only at the final division; covered by `GetAverageScoreMatchesDoublePrecisionAccumulation`. (`src/domain/user_profile.cpp:96`, `UserProfile::getAverageScore`)
...
- [x] Profile-store saves truncate the destination directly and ignore open/write/serialization failures. (`src/data/formats/protobuf/profile_serializer.cpp:15`, `ProfileSerializer::save`)
...
- [x] The session-controller fake ignores `getMostRecentPerfs` count and shares load/build observability. `getMostRecentPerfs` now honors `count`, covered by `FakeProfileServiceGetMostRecentPerfsHonorsRequestedCount`; the load/build-observability half is unaddressed. (`tests/app/session_controller_test.cpp:48`, `FakeProfileService::getMostRecentPerfs`)
- [x] `onCurrentPerfChanged` behaviour is covered only by a no-op test double. Stale: already covered by the real App wiring in `CompositionRootTest.CurrentPerfChangeCascadesToGraphViewModelReload` (`tests/integration/composition_root_test.cpp`). (`tests/ui/graph_vm_test.cpp:27`, `FakeGraphUseCase::onCurrentPerfChanged`)
...
- [x] `FileService` retains a temporary watcher diagnostic loop. Stale: removed in `c18afb7`. (`src/qt_data/file_service.cpp:42`, `FileService::handleDirectoryChanged`)
...
- [x] `runAt()` returns a pointer into a replaceable vector. Stale: `RunListModel::runAt()` no longer exists. (`src/ui/presentation/run_list_model.cpp:49`, `RunListModel::runAt`)
- [x] QML can request `ScenarioPerf` by value without a declared metatype contract. Already fixed: `SessionViewModel::getCurrentPerf()` is no longer `Q_INVOKABLE`; only the `QString`-returning `getCurrentPerfScenario()` is exposed to QML. (`src/ui/presentation/session_vm.h:29`, `SessionViewModel::getCurrentPerf`)
...
- [x] Integration tests use divergent manual QML registration. (`tests/integration/qt_test_main.cpp:16`, `registerQmlTypes`; `src/qml_registration.h:15`, `declare_metatypes`)
...
- [x] Commented-out code lacks the required dated TODO marker. (`CMakeLists.txt:109`; `src/ui/qml/AppMenuBar.qml:29`)

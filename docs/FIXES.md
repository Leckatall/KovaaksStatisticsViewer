...
- [ ] Invalid `add_data` input can create a phantom zero data point before throwing. (`src/domain/scenario_perf.cpp:11`, `ScenarioPerf::add_data`; `src/domain/scenario_perf.cpp:43`, `ScenarioPerf::get_data_point`)
...
- [ ] `GraphUseCase::load_perf()` passes `string_view::data()` to a length-owning string API. (`src/app/usecases/graph_use_case.h:18`, `GraphUseCase::load_perf`)
...
- [ ] `GraphUseCase` retains a console diagnostic. (`src/app/usecases/graph_use_case.h:19`, `GraphUseCase::load_perf`)

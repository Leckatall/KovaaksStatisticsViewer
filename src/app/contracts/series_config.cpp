...
namespace ksv::application {
    namespace {
        ...
        void validatePresentation(const SeriesPresentation &presentation, const bool computed,
            ...
            if (!std::isfinite(presentation.lineStyle.width)) {
                errors.push_back({SeriesConfigValidationCode::NonFiniteLineWidth,
                                  std::string(path) + ".lineStyle.width"});
            } else if (presentation.lineStyle.width < kMinimumLineWidth || presentation.lineStyle.width > kMaximumLineWidth) {
                errors.push_back({SeriesConfigValidationCode::LineWidthOutOfRange,
                                  std::string(path) + ".lineStyle.width"});
            }
            if (!computed) return;
                    ...
                    std::visit([&](const auto &selection) {
                        ...
                        if constexpr (std::same_as<Selection, RecentRuns>) {
                            if (selection.count == 0) {
                                errors.push_back({SeriesConfigValidationCode::InvalidRecentRunCount, path + ".selection.count"});
                            }
                        } else if (!std::isfinite(selection.percent) || selection.percent <= 0.0 || selection.percent > 100.0) {
                            errors.push_back({SeriesConfigValidationCode::InvalidTopPercentile, path + ".selection.percent"});
                        }
                    }, node.selection);
                    validateInput(node.input, "input");
                }
            ...
            std::visit([&](const auto &series) {
                ...
                if constexpr (std::same_as<Config, BaseSeriesConfig>) {
                    if (!isKnownPrimitiveMetric(series.metric)) {
                        errors.push_back({SeriesConfigValidationCode::InvalidPrimitiveMetric, std::string(root) + ".metric"});
                    }
                } else {
    ...
    Expression subtract(Expression left, Expression right) { return makeExpression(Subtract{std::move(left), std::move(right)}); }
    Expression multiply(Expression left, Expression right) { return makeExpression(Multiply{std::move(left), std::move(right)}); }
    Expression divide(Expression left, Expression right) { return makeExpression(Divide{std::move(left), std::move(right)}); }
    Expression runningSum(Expression input) { return makeExpression(RunningSum{std::move(input)}); }
    Expression rollingMean(Expression input, const uint32_t window) { return makeExpression(RollingMean{std::move(input), window}); }
    ...
    std::vector<ValidationError> validateSeriesConfigs(const std::vector<SeriesConfig> &configs) {
        ...
        for (size_t index = 0; index < configs.size(); ++index) {
            ...
            errors.insert(errors.end(), std::make_move_iterator(recordErrors.begin()), std::make_move_iterator(recordErrors.end()));
                ...
                if constexpr (std::same_as<Config, BaseSeriesConfig>) {
                    ...
                    if (metric != kPrimitiveMetrics.end()) {
                        ...
                        if (series.presentation.name != canonicalName(series.metric)) {
                            errors.push_back({SeriesConfigValidationCode::NonCanonicalBaseName, root + ".presentation.name"});
                        }
                    }
        ...
        for (size_t index = 0; index < baseCounts.size(); ++index) {
            if (baseCounts[index] == 0) {
                errors.push_back({SeriesConfigValidationCode::MissingBaseMetric,
        ...
        for (size_t index = 1; index < computedIds.size(); ++index) {
            if (computedIds[index].first == computedIds[index - 1].first) {
                errors.push_back({SeriesConfigValidationCode::DuplicateComputedSeriesId,
        ...
        for (size_t index = 0; index < positions.size(); ++index) {
            if (index > 0 && positions[index].first == positions[index - 1].first) {
                errors.push_back({SeriesConfigValidationCode::DuplicateDisplayPosition,
                                  "records[" + std::to_string(positions[index].second) + "].presentation.displayPosition"});
            }
        }
        for (size_t index = 0; index < positions.size(); ++index) {
            if (positions[index].first != index) {
                errors.push_back({SeriesConfigValidationCode::NonDenseDisplayPosition,
                                  "records[" + std::to_string(positions[index].second) + "].presentation.displayPosition"});
                break;
            }
        }
    ...
    std::vector<SeriesConfig> defaultSeriesConfigs() {
        ...
        return {
            ...
            ComputedSeriesConfig{ComputedSeriesId{1}, {"Accuracy", line({0, 255, 255, 255}), true, 1},
            ...
            ComputedSeriesConfig{ComputedSeriesId{2}, {"Score Total", line({128, 0, 128, 255}), true, 6},
                                 runningSum(primitive(PrimitiveMetric::Score))},
            ComputedSeriesConfig{ComputedSeriesId{3}, {"Expected Final Score", line({255, 0, 255, 255}), true, 7},
                                 projectedFinalValue(primitive(PrimitiveMetric::Score))},
            ComputedSeriesConfig{ComputedSeriesId{4}, {"Expected Final Score (5s)", line({0, 191, 255, 255}), true, 8},
                                 projectedFinalValue(rollingMean(primitive(PrimitiveMetric::Score), 5))}
        };
    }
}

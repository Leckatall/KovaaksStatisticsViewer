...
namespace ksv::domain {
    ...
    public:
        ...
        bool addRun(const Run &run);

        [[nodiscard]] std::vector<ScenarioId> getScenarioList() const;

        [[nodiscard]] std::optional<Run> getLatestRun() const;

        [[nodiscard]] std::optional<Run> getMostRecentRun(const ScenarioId &scenario) const;

        [[nodiscard]] std::vector<Run> getMostRecentRuns(const ScenarioId &scenario, std::size_t count) const;

        [[nodiscard]] std::vector<Run> getRunsForScenario(const ScenarioId &scenario) const;

        [[nodiscard]] std::vector<RunSummary> getCompletionHistory(const ScenarioId &scenario) const;

        [[nodiscard]] std::optional<float> getAverageScore(const ScenarioId &scenario, std::size_t count) const;

        [[nodiscard]] std::optional<Run> getCurrentRun(const ScenarioRunId &run_id) const;

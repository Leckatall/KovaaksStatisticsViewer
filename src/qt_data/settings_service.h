...
namespace ksv::qt_data {
    class SettingsService: public QObject, public application::ISettingsService {
        ...
        [[nodiscard]] std::vector<std::string> getKovaaksDirs() const override;
        ...
        void onKovaaksDirsChanged(std::function<void()> callback) override {
            m_kovaaks_dirs_callbacks.push_back(std::move(callback));
        }
    private:
        [[nodiscard]] std::string readDirSetting(const QString &key, const QVariant &default_value) const;
        [[nodiscard]] std::vector<std::string> readDirListSetting(const QString &key) const;

...
namespace {
    ...
    TEST_F(PlaytimeGraphViewModelTest, RefreshEmitsPointCountAndBoundsChanged) {
        ...
        const QSignalSpy pointSpy(&view_model, &GraphViewModelBase::pointCountChanged);
        const QSignalSpy boundsSpy(&view_model, &GraphViewModelBase::boundsChanged);
        view_model.refresh();

        EXPECT_GT(pointSpy.count(), 0);
        EXPECT_GT(boundsSpy.count(), 0);
    }

    TEST_F(PlaytimeGraphViewModelTest, FormatXTickRendersTheEpochDayAsACalendarDate) {
        ...
        EXPECT_EQ(view_model.formatXTick(dayValue), expected);
